...
The main ChatGPT entry control is a toggle, not an idempotent open command. Treat both the presence and absence of `aria-pressed` as stale hints, never as ownership evidence. If the exact task-owned dialog is already visible, do not click; if no owned dialog or active ChatGPT document exists, a bounded refresh may click even when the button still claims it is pressed. After send, preserve one atomic lease consisting of the exact conversation identity plus original marker: verify it before any entry-control action and on every collection poll. Reopen the entry only when that lease is not currently visible, then require the same lease again before collecting.

After a completed main-surface job, the client may asynchronously unmount that dialog while the next fresh job is entering its `新聊天` gate. Fresh preparation must therefore refresh the idempotent entry inside the bounded new-conversation wait, with a cooldown that prevents rapid toggle clicks. It must still prove a blank exact conversation identity before send; a later job that never reaches that gate remains `not-submitted`.
