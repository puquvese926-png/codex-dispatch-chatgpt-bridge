...
The watch manifest contains `schemaVersion`, the pinned `conversationId`, `surface: "chatgpt-main-chat"`, and an absolute `checkpointPath`; watch passes that validated manifest surface to every read. A successful handoff is a cross-process transaction: the watcher acquires the unique adjacent `<checkpointPath>.lock` by atomic exclusive directory creation, writes a lease containing a random owner ID, PID, exact checkpoint path and acquisition time, then re-reads and strictly validates the checkpoint from disk while holding the lock. It selects the next approved handoff from that fresh state, records the task ID and normalized plan hash with the hardened adjacent-temp atomic JSON writer, and only then returns `handoff-ready`; the owner-checked `finally` release is mandatory. A second watcher that observed the same proposal before locking must re-read the committed checkpoint and return `no-new-delivery`, never a second ready result. A live lock is bounded-wait/busy, a young incomplete or malformed lease is busy, and only a clearly stale lease whose owner is absent (or clearly stale incomplete metadata) may be reclaimed. Corrupt, rebound or identity-mismatched checkpoints fail closed and are never replaced. If report writing fails after the checkpoint commit, the checkpoint remains the deduplication fact and a later watcher must not redeliver. This local checkpoint mutation is not a ChatGPT history mutation. Identity mismatch, unreadable surface, malformed protocol, changed-plan task ID reuse, lock contention at the deadline and timeout all fail closed without sending.
